...
// (Mantengo la función normalize_banda_config de backup)
...
// Detecta posibles handles (tu función original, sin cambios lógicos)
...
// Localización mejorada: intenta inyectar inline BEFORE el script detectado (evita race)
function banda_localize_pricing_script_improved() {
    ...
    // Preparar datos de localización - FORZAR VALORES POR DEFECTO PARA USUARIOS SIN MEMBRESÍA ACTIVA
    $current_storage = '1tb';
    $current_users = 2;
    ...
    if (is_user_logged_in()) {
        ...
        if ($has_banda_membership && $has_active_membership) {
            ...
            if (!empty($config_json)) {
                ...
                if (is_array($config) && json_last_error() === JSON_ERROR_NONE && !isset($config['auto_created'])) {
                    ...
                    $current_users = isset($config['num_users']) ? (int)$config['num_users'] : 2;
                    $current_frequency = $config['payment_frequency'] ?? 'monthly';
                    $has_previous_config = true;
                }
            }

        // AGREGAR ESTOS DATOS PARA PRORRATEO
        ...
        if (function_exists('pmpro_getMembershipLevelForUser')) {
            ...
            if (!empty($level) && !empty($level->enddate) && $level->enddate !== '0000-00-00 00:00:00') {
                $next_payment_date = date('c', strtotime($level->enddate));
            }
        }
    }
	...
	$localization = [
		...
		'current_subscription_data' => $current_subscription_data, // AGREGAR ESTA LÍNEA
    ...
    // Si no se pudo inyectar 'before' en ninguno de los handles detectados, mantener los fallbacks (wp_head/wp_footer)
    if (!$localized) {
        ...
        add_action('wp_head', function() use ($json) {
            ...
            // no log aquí porque puede correr muchas veces
        }, 1);
...
// Hooks para ejecutar la localización en tiempo (con prioridad para esperar registros de scripts)
...
function banda_theme_log($message, $context = []) {
    if (function_exists('nextcloud_banda_log_info')) {
        ...
        if (!empty($context)) $log_message .= ' | ' . wp_json_encode($context, JSON_UNESCAPED_UNICODE);
