...
// Asegurar que tengamos los snippets
if (!isset($snippets)) {
                ...
                <?php foreach ($snippets as $id => $snippet): ?>
                    <tr>
                            ...
                            <div class="row-actions">
                                ...
                                <?php if (isset($snippet['active']) && $snippet['active']): ?>
                                    <span class="deactivate">
                                        <a href="<?php echo esc_url(wp_nonce_url(admin_url('admin-post.php?action=deactivate_snippet&id=' . $id), 'toggle_snippet')); ?>" class="submitdelete sc-confirm-delete">
                        ...
                        <td class="column-description" data-colname="<?php esc_attr_e('Description', 'simply-code'); ?>">
                            <?php echo esc_html(isset($snippet['description']) ? $snippet['description'] : __('No description', 'simply-code')); ?>
                        </td>
                        ...
                        <td class="column-date" data-colname="<?php esc_attr_e('Modified', 'simply-code'); ?>">
                            <?php 
                            $modified = isset($snippet['updated_at']) ? $snippet['updated_at'] : (isset($snippet['created_at']) ? $snippet['created_at'] : '');
                            if ($modified) {
                        ...
                        <td class="column-actions" data-colname="<?php esc_attr_e('Actions', 'simply-code'); ?>">
                            <a href="<?php echo esc_url(admin_url('admin.php?page=simply-code-editor&action=edit&id=' . $id)); ?>" class="button button-small">
                                <?php _e('Edit', 'simply-code'); ?>
                            </a>
                        </td>
                    </tr>
...
.submitdelete.sc-confirm-delete {
    color: #d63638;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.sc-confirm-delete').forEach(function(element) {
        element.addEventListener('click', function(e) {
            if (!confirm('<?php echo esc_js(__('Are you sure you want to delete this snippet?', 'simply-code')); ?>')) {
                e.preventDefault();
            }
        });
    });
